...
from Dataset import MTLTranSynDataset
from torch.nn import MSELoss
import torch
import time
import pandas as pd
from utils.tools import EarlyStopping, set_seed, filter1, CategoricalCrossEntropyLoss
from static.constant import Fold, DrugAE_SaveBase, CellAE_SaveBase, MTLTranSyn_SaveBase, MTLTranSyn_Result, \
    DrugAE_OutputDim, CellAE_OutputDim
from Models import MTLTranSyn, DrugAE, CellLineAE
import numpy as np
...
def test(model, drugEncoder, cellLineEncoder, test_dataloader, test_num, mse, cce, test_fold):
    ...
    with torch.no_grad():
        ...
        print("test result:\n" + "Synergy:\n" + str(synergy_result) + "\nSensitivity:\n" + str(
            sensitivity_result) + "\n")
        with open(MTLTranSyn_Result, 'a') as file:
            file.write("---------- Test ----------\n")
...
cellLineAE.eval()
print(
    "--------- MTLTranSyn with drugAE(" + str(DrugAE_OutputDim) + ") cellLineAE(" + str(
        CellAE_OutputDim) + ") ---------")
with open(MTLTranSyn_Result, 'a') as file:
    file.write("--------- MTLTranSyn with drugAE(" + str(DrugAE_OutputDim) + ") cellLineAE(" + str(
        CellAE_OutputDim) + ") ---------\n")
result_per_fold = []
...
for fold_test in range(0, Fold):
    ...
    hp_i = 0
    result_per_hp = []
    save_path = MTLTranSyn_SaveBase + "fold_" + str(fold_test) + ".pth"

    best_hp = hyper_parameters_candidate[0]
    ...
    train_dataset = MTLTranSynDataset(drugs, cell_lines, train_summary)
    train_num = len(train_dataset)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                              num_workers=2, pin_memory=True)
    validation_dataset = MTLTranSynDataset(drugs, cell_lines, validation_summary)
    validation_num = len(validation_dataset)
    validation_loader = DataLoader(validation_dataset, batch_size=batch_size,
                                   shuffle=False)  # no shuffle
    test_dataset = MTLTranSynDataset(drugs, cell_lines, test_summary)
    test_num = len(test_dataset)
    test_loader = DataLoader(test_dataset, batch_size=batch_size,
                             shuffle=False)  # no shuffle

    model = MTLTranSyn(best_hp['hidden_neurons'], model_input).to(device)
...
with open(MTLTranSyn_Result, 'a') as file:
    file.write("\n")
totaltime = time.time() - statime
print(f"总共花费了{totaltime / 3600} h")
